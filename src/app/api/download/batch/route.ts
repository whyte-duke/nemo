...
import type { BatchDownloadRequest } from "@/types/download";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const body: BatchDownloadRequest & {
      quality?: string;
    ...
    const { data: queueEntry, error: dbError } = await supabase
      ...
      .insert({
        ...
        selected_indices: body.reference_indices,
        destination_path: body.destination_dir,
    ...
    if (dbError) {
      console.error("[download/batch] DB error:", dbError);
    ...
    const payload = {
      urls: body.urls,
      reference_indices: body.reference_indices,
    ...
    try {
      await fetch(`${DOWNLOAD_API}/batch-download`, {
        method: "POST",
        headers: {
      ...
      console.error("[download/batch] Backend unreachable:", fetchErr);
      await supabase
        .from("download_queue")
    ...
    return NextResponse.json({
      status: "success",
      message: "Batch ajouté à la file d'attente",
      download_id: entry.id,
    });
  } catch (err) {
    console.error("[download/batch]", err);
